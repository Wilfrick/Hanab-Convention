...
3. forall h forall n if card[h][n] == T && != IP => card[h][N := min{n:card[h][n] = T && != IP}] + IP, if card[h][N] == CH => card[h][N] -= CH, if N-1 >= 0 => card[h][N-1] + CH, break n; forall h forall n card[h][n] - T; ("Frontmost newly clued cards are immediately playable and are no longer chop cards if they were before. This is a play clue and nothing else.")
